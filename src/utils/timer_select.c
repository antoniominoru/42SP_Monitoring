...
/*   Updated: 2022/08/02 21:08:50 by aminoru-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	timer_selected(t_list *list, int timer)
{
	...
	while (ft_lstsize(temp))
	{
		...
		if (!ft_strncmp(teste[1], "DNS", ft_strlen(teste[1])))
			...
			{
				system("echo ---------------- >> log/monitoring.log");
				system("nslookup intra.42.fr 8.8.8.8 2>&1 | sed '/^$/d'| sed \"3s/^/Monitoramento: DNS /\" | tee -a log/monitoring.log | grep Monitoramento:");
			}
		temp = temp->next;
	}
}
